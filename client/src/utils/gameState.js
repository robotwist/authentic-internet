...
export const validateGameState = (state) => {
  ...
  // Validate map index
  if (state.currentMapIndex < 0 || state.currentMapIndex >= MAPS.length)
    return false;
...
export const mergeArtifacts = (
  ...
  console.log("Merging artifacts:", {
    mapArtifacts: mapArtifacts?.map((a) => ({
      id: a.id,
      name: a.name,
      location: a.location,
    })),
    userArtifacts: userArtifacts?.map((a) => ({
      id: a.id,
      name: a.name,
      location: a.location,
    })),
    modifiedArtifacts: modifiedArtifacts?.map((a) => ({
      id: a.id,
      name: a.name,
      location: a.location,
    })),
  ...
  (userArtifacts || []).forEach((userArtifact) => {
    if (!artifacts.find((a) => a.id === userArtifact.id)) {
      artifacts.push({
        ...userArtifact,
        location: userArtifact.location || {
          x: userArtifact.x,
          y: userArtifact.y,
        },
      });
...
export const updateArtifact = (artifact, updates, userModifiable) => {
  ...
  if (userModifiable.unlockAnswer)
    allowedUpdates.unlockAnswer = updates.unlockAnswer;
