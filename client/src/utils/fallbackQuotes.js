/**
 * Fallback quotes module - provides quotes when API calls fail
 */

// Shakespeare quotes
export const SHAKESPEARE_QUOTES = [
  {
    text: "To be, or not to be, that is the question.",
    author: "William Shakespeare",
    source: "Hamlet",
    type: "shakespeare"
  },
  {
    text: "All the world's a stage, and all the men and women merely players.",
    author: "William Shakespeare",
    source: "As You Like It",
    type: "shakespeare"
  },
  {
    text: "Love all, trust a few, do wrong to none.",
    author: "William Shakespeare",
    source: "All's Well That Ends Well",
    type: "shakespeare"
  },
  {
    text: "The fool doth think he is wise, but the wise man knows himself to be a fool.",
    author: "William Shakespeare",
    source: "As You Like It",
    type: "shakespeare"
  },
  {
    text: "We know what we are, but know not what we may be.",
    author: "William Shakespeare",
    source: "Hamlet",
    type: "shakespeare"
  },
  {
    text: "It is not in the stars to hold our destiny but in ourselves.",
    author: "William Shakespeare",
    source: "Julius Caesar",
    type: "shakespeare"
  },
  {
    text: "The course of true love never did run smooth.",
    author: "William Shakespeare",
    source: "A Midsummer Night's Dream",
    type: "shakespeare"
  },
  {
    text: "There is nothing either good or bad, but thinking makes it so.",
    author: "William Shakespeare",
    source: "Hamlet",
    type: "shakespeare"
  }
];

// Zen wisdom quotes
export const ZEN_QUOTES = [
  {
    text: "When you realize nothing is lacking, the whole world belongs to you.",
    author: "Lao Tzu",
    type: "zen"
  },
  {
    text: "The journey of a thousand miles begins with a single step.",
    author: "Lao Tzu",
    type: "zen"
  },
  {
    text: "Before enlightenment, chop wood, carry water. After enlightenment, chop wood, carry water.",
    author: "Zen Proverb",
    type: "zen"
  },
  {
    text: "The mind is everything. What you think you become.",
    author: "Buddha",
    type: "zen"
  },
  {
    text: "If a problem has a solution, why worry? If it doesn't, worrying won't help.",
    author: "Shantideva",
    type: "zen"
  },
  {
    text: "Peace comes from within. Do not seek it without.",
    author: "Buddha",
    type: "zen"
  },
  {
    text: "What you seek is seeking you.",
    author: "Rumi",
    type: "zen"
  },
  {
    text: "The obstacle is the path.",
    author: "Zen Proverb",
    type: "zen"
  }
];

// John Muir quotes
export const JOHN_MUIR_QUOTES = [
  {
    text: "The mountains are calling and I must go.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "In every walk with nature one receives far more than he seeks.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "The clearest way into the Universe is through a forest wilderness.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "Between every two pines is a doorway to a new world.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "The power of imagination makes us infinite.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "When one tugs at a single thing in nature, he finds it attached to the rest of the world.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "Climb the mountains and get their good tidings. Nature's peace will flow into you as sunshine flows into trees.",
    author: "John Muir",
    type: "john_muir"
  },
  {
    text: "Of all the paths you take in life, make sure a few of them are dirt.",
    author: "John Muir",
    type: "john_muir"
  }
];

// Philosopher quotes
export const PHILOSOPHER_QUOTES = [
  {
    text: "The unexamined life is not worth living.",
    author: "Socrates",
    type: "philosopher"
  },
  {
    text: "Happiness is not something ready-made. It comes from your own actions.",
    author: "Dalai Lama",
    type: "philosopher"
  },
  {
    text: "We are what we repeatedly do. Excellence, then, is not an act, but a habit.",
    author: "Aristotle",
    type: "philosopher"
  },
  {
    text: "I think, therefore I am.",
    author: "René Descartes",
    type: "philosopher"
  },
  {
    text: "He who is unable to live in society, or who has no need because he is sufficient for himself, must be either a beast or a god.",
    author: "Aristotle",
    type: "philosopher"
  },
  {
    text: "The life of man is solitary, poor, nasty, brutish, and short.",
    author: "Thomas Hobbes",
    type: "philosopher"
  },
  {
    text: "To live is to suffer, to survive is to find some meaning in the suffering.",
    author: "Friedrich Nietzsche",
    type: "philosopher"
  },
  {
    text: "The only thing I know is that I know nothing.",
    author: "Socrates",
    type: "philosopher"
  }
];

// General daily quotes
export const DAILY_QUOTES = [
  {
    text: "Be yourself; everyone else is already taken.",
    author: "Oscar Wilde",
    type: "daily"
  },
  {
    text: "Two things are infinite: the universe and human stupidity; and I'm not sure about the universe.",
    author: "Albert Einstein",
    type: "daily"
  },
  {
    text: "Be the change that you wish to see in the world.",
    author: "Mahatma Gandhi",
    type: "daily"
  },
  {
    text: "In three words I can sum up everything I've learned about life: it goes on.",
    author: "Robert Frost",
    type: "daily"
  },
  {
    text: "If you tell the truth, you don't have to remember anything.",
    author: "Mark Twain",
    type: "daily"
  },
  {
    text: "Always forgive your enemies; nothing annoys them so much.",
    author: "Oscar Wilde",
    type: "daily"
  },
  {
    text: "Live as if you were to die tomorrow. Learn as if you were to live forever.",
    author: "Mahatma Gandhi",
    type: "daily"
  },
  {
    text: "Without music, life would be a mistake.",
    author: "Friedrich Nietzsche",
    type: "daily"
  }
];

// Helper function to get a random quote from an array
export const getRandomQuoteFromArray = (quoteArray) => {
  if (!quoteArray || !quoteArray.length) {
    return {
      text: "Wisdom awaits those who seek it.",
      author: "Anonymous",
      type: "daily"
    };
  }
  
  return quoteArray[Math.floor(Math.random() * quoteArray.length)];
};

// Function to get quotes based on type
export const getQuotesByType = (type) => {
  switch (type) {
    case 'shakespeare':
      return SHAKESPEARE_QUOTES;
    case 'zen':
      return ZEN_QUOTES;
    case 'john_muir':
      return JOHN_MUIR_QUOTES;
    case 'philosopher':
      return PHILOSOPHER_QUOTES;
    case 'daily':
    default:
      return DAILY_QUOTES;
  }
};

// Function to get a combined array of all quotes
export const getAllQuotes = () => {
  return [
    ...SHAKESPEARE_QUOTES,
    ...ZEN_QUOTES,
    ...JOHN_MUIR_QUOTES,
    ...PHILOSOPHER_QUOTES,
    ...DAILY_QUOTES
  ];
}; 