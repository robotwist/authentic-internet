...
class ErrorMonitor {
  ...
  getSolution(type, details) {
    ...
    if (type === 'fetch error' || type === 'xhr error') {
      if (details.status === 403 || (details.message && details.message.includes('CORS'))) {
    ...
    // Network error
    if ((type === 'fetch error' || type === 'xhr error') && 
        ((details.message && details.message.includes('Network')) || details.status === 0)) {
      return {
        title: 'Network Error',
    ...
    // API errors
    if ((type === 'fetch error' || type === 'xhr error') && details.status >= 400) {
