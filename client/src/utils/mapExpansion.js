...
export function expandMap(originalMap, targetRows, targetCols, options = {}) {
  ...
  const startRow = centerOriginal ? Math.floor((targetRows - originalRows) / 2) : 0;
  const startCol = centerOriginal ? Math.floor((targetCols - originalCols) / 2) : 0;
  ...
  for (let r = 0; r < originalRows; r++) {
    for (let c = 0; c < originalCols; c++) {
      ...
      if (newRow >= 0 && newRow < targetRows && newCol >= 0 && newCol < targetCols) {
...
export function validateMap(map, expectedRows, expectedCols) {
  ...
  if (map.length !== expectedRows) {
    return { valid: false, error: `Expected ${expectedRows} rows, got ${map.length}` };
  }
  for (let i = 0; i < map.length; i++) {
    ...
    if (map[i].length !== expectedCols) {
      return { valid: false, error: `Row ${i}: expected ${expectedCols} cols, got ${map[i].length}` };
    }
  }
  return { valid: true };
}
