...
const CollaborationPage = () => {
  ...
  const joinSession = async () => {
    try {
      const response = await collaborationAPI.joinSession(joinSessionData.sessionId, joinSessionData.role);
      setSessions(prev => [response.collaboration, ...prev]);
  ...
  const getParticipantRole = (session) => {
    ...
    const participant = session.participants.find(p => p.user._id === user._id);
  ...
  const filteredSessions = sessions.filter(session => {
    ...
    const matchesType = typeFilter === 'all' || session.artifactType === typeFilter;
    ...
    <Box className="collaboration-page">
                    ...
                    <CardContent>
                      ...
                      <Box display="flex" gap={1} mb={2}>
                        ...
                        <Chip
                          ...
                          size="small"
                          color={getParticipantRole(session) === 'Owner' ? 'primary' : 'default'}
                        />
                      </Box>
      ...
      <Dialog open={showCreateDialog} onClose={() => setShowCreateDialog(false)} maxWidth="md" fullWidth>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setNewSessionData(prev => ({ ...prev, artifactType: e.target.value }))}
                  label="Artifact Type"
                >
                  <MenuItem value="GAME">Game</MenuItem>
                  <MenuItem value="STORY">Story</MenuItem>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setNewSessionData(prev => ({
                    ...prev,
                    settings: { ...prev.settings, maxParticipants: e.target.value }
                  }))}
        ...
        <DialogActions>
          ...
          <Button onClick={createSession} variant="contained" disabled={!newSessionData.name}>
            Create Session
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={showJoinDialog} onClose={() => setShowJoinDialog(false)} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) => setJoinSessionData(prev => ({ ...prev, sessionId: e.target.value }))}
                required
              />
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setJoinSessionData(prev => ({ ...prev, role: e.target.value }))}
                  label="Role"
                >
        ...
        <DialogActions>
          ...
          <Button onClick={joinSession} variant="contained" disabled={!joinSessionData.sessionId}>
            Join Session
          </Button>
        </DialogActions>
      </Dialog>
