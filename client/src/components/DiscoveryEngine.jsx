...
const DiscoveryEngine = ({ artifacts, onArtifactSelect, loading = false }) => {
  ...
  const calculateEngagementScore = (artifact) => {
    ...
    if (daysSinceCreation <= 7) score += 20; // New content bonus
  ...
  const personalizedRecommendations = useMemo(() => {
    ...
    const scoredArtifacts = artifacts
      .filter(artifact => !viewedArtifacts.includes(artifact.id || artifact._id))
      .map(artifact => ({
        ...artifact,
        recommendationScore: calculateRecommendationScore(artifact, userBehavior)
      }))
  ...
  const calculateRecommendationScore = (artifact, userBehavior) => {
    ...
    // Creator preference
    if (artifact.createdBy && userBehavior.preferredCreators[artifact.createdBy]) {
  ...
  const popularCreators = useMemo(() => {
    ...
    artifacts.forEach(artifact => {
      if (artifact.createdBy) {
        ...
        creatorStats[artifact.createdBy].totalReviews += artifact.reviews?.length || 0;
      }
    });
    ...
    return Object.values(creatorStats)
      .map(creator => ({
        ...creator,
        averageRating: creator.artifactCount > 0 ? creator.totalRating / creator.artifactCount : 0,
        popularityScore: creator.artifactCount * 2 + creator.totalReviews * 3 + creator.averageRating * 10
      }))
                      ...
                      <CardContent>
                        <Box className="creator-header">
                          ...
                          <Box className="creator-info">
                            ...
                            <Typography variant="body2" className="creator-stats">
                              {creator.artifactCount} artifacts • {creator.averageRating.toFixed(1)} ⭐
                            </Typography>
                          </Box>
                        </Box>
                        ...
                        <Box className="creator-actions">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const creatorArtifacts = artifacts.filter(a => a.createdBy === creator.name);
                ...
                {trendingTopics.map((topic, index) => (
                  <Chip
                    ...
                    onClick={() => {
                      ...
                      const topicArtifacts = artifacts.filter(a => 
                        a.tags && a.tags.includes(topic.name)
                      );
                      // You could implement a callback to show these artifacts
                    }}
                  />
