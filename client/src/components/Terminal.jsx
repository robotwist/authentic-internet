...
const Terminal = ({
    ...
    pwd: {
      ...
      action: () => {
        return [
          {
            type: "system",
            content: `/home/${user.split("@")[0]}/${directory}`,
          },
        ];
      },
    },
      ...
      if (isComplete) {
        ...
        setOutput((prev) => [
          ...
          { type: "system", content: "✅ Challenge completed!" },
          {
            type: "system",
            content:
              terminalData.winMessage ||
              "You have successfully completed this terminal challenge.",
          },
        ]);
    ...
    if (e.key === "ArrowUp") {
      ...
      if (cmdStart) {
        const matches = Object.keys(commands).filter(
          (cmd) => cmd.startsWith(cmdStart) && cmd !== cmdStart,
        );
  ...
  const handleSubmit = (e) => {
    ...
    setOutput((prev) => [
      ...prev,
      { type: "command", content: commandWithPrompt },
    ]);
