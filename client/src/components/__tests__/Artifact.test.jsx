...
describe('Artifact Component', () => {
  ...
  test('renders artifact with basic information', () => {
    ...
    expect(screen.getByText('A test artifact for testing purposes')).toBeInTheDocument();
  });
  ...
  test('handles collect action', () => {
    ...
    expect(defaultProps.onArtifactCollect).toHaveBeenCalledWith('test-artifact');
  });
  ...
  test('applies correct CSS classes based on rarity', () => {
    const { rerender } = renderArtifact({ artifact: { ...mockArtifact, rarity: 'legendary' } });
    
    const artifactElement = screen.getByTestId('artifact-test-artifact');
    expect(artifactElement).toHaveClass('artifact', 'legendary');
    
    rerender(<Artifact {...defaultProps} artifact={{ ...mockArtifact, rarity: 'epic' }} />);
