...
const NPC = ({
  ...
  const getCharacterSpecificErrorMessage = useCallback((npcType) => {
    const messages = {
      ...
      [NPC_TYPES.ADA_LOVELACE]:
        "The science of operations, as derived from mathematics more especially, is a science of itself, and has its own abstract truth and value. (Notes on the Analytical Engine, 1843)",
      ...
      [NPC_TYPES.ARTIST]:
        "The art speaks for itself, but sometimes it needs a translator.",
      ...
      [NPC_TYPES.ZEUS]:
        "Even the gods cannot alter the past, but the future is yet in my power. (Greek mythology)",
      [NPC_TYPES.JOHN_MUIR]:
  ...
  const greetings = useMemo(
    () => ({
      ...
      [NPC_TYPES.ARTIST]:
        "Welcome! Let us explore the intersection of art and technology.",
      [NPC_TYPES.MICHELANGELO]:
      ...
      [NPC_TYPES.ZEUS]:
        "I am the Thunderer! Here in my cloud-girded hall, what mortal dares challenge the might of Zeus? (Iliad, 8th century BCE)",
      [NPC_TYPES.JOHN_MUIR]:
        "The mountains are calling and I must go. (Letter to his sister Sarah Muir, 1873)",
      [NPC_TYPES.JESUS]:
        ...
        <div key={index} className={messageClasses}>
          {isNPC && (
            <div
              ...
              style={{ backgroundImage: `url(${getNPCImage()})` }}
            ></div>
          )}
              ...
              <div className="message-actions">
                <button
                  ...
                  title={
                    isAlreadySaved
                      ? "Quote already saved"
                      : "Save this quote to your collection"
                  }
                ...
                {shareAvailable && (
                  <button
                    ...
                    onClick={() =>
                      handleShareQuote(message.text, message.source)
                    }
  ...
  return (
    <div>
      ...
      {showDialog && (
        <div
          className={`dialog-overlay ${keyboardVisible ? "keyboard-open" : ""} ${isMobile ? "mobile" : ""}`}
        >
