...
const GameWorld = React.memo(() => {
  ...
  const [characterPosition, setCharacterPosition] = useState(INITIAL_CHARACTER_POSITION);
  ...
  // Memoized current map data
  const currentMap = useMemo(() => MAPS[currentMapIndex] || MAPS[0], [currentMapIndex]);
  const currentMapNPCs = useMemo(() => currentMap?.npcs || [], [currentMap]);
  const currentMapArtifacts = useMemo(() => currentMap?.artifacts || [], [currentMap]);
  ...
  useEffect(() => {
    ...
    if (renderTime > 16) { // Longer than 60fps threshold
      console.warn(`Slow render detected: ${renderTime.toFixed(2)}ms (render #${renderCount.current})`);
    }
    
    lastRenderTime.current = now;
    
    // Performance mark for render
    if (performance.mark) {
      performance.mark(PERFORMANCE_MARKERS.RENDER_END);
      performance.measure('GameWorld Render', PERFORMANCE_MARKERS.RENDER_START, PERFORMANCE_MARKERS.RENDER_END);
    }
  });
  ...
  const handlePortalTransition = useCallback((destinationMap, spawnPosition, isSpecial = false) => {
    ...
    const transitionAnimation = () => {
      setTimeout(() => {
        ...
        performance.measure('Portal Transition', PERFORMANCE_MARKERS.PORTAL_TRANSITION, PERFORMANCE_MARKERS.PORTAL_TRANSITION + '-end');
      }, 1000);
    };
    
    requestAnimationFrame(transitionAnimation);
  }, [portalState.isTransitioning, soundManager, setCurrentSpecialWorld, updatePortalState]);

  // Optimized portal activation handler
  const handlePortalActivation = useCallback((e) => {
    ...
    if (progressionPortal) {
      ...
      if (!condition || condition(characterPosition.x / TILE_SIZE, characterPosition.y / TILE_SIZE)) {
    ...
    if (currentMapName === 'Yosemite') {
      ...
      const specialPortal = PORTAL_CONFIG.special[portalX];
      if (specialPortal && portalY === 1) {
        updatePortalState({ 
  ...
  }, [portalState.isTransitioning, currentMap, characterPosition, PORTAL_CONFIG, handlePortalTransition, updatePortalState]);
  ...
  }, [gameData.levelCompletion, updateGameState, updateNotifications, soundManager, user, checkLevelAchievements]);
  ...
  }, [currentMapIndex, characterPosition, user, gameData.artifacts, soundManager, updateGameState]);
    ...
    switch (event.key) {
      ...
      case 'I':
        ...
        if (mobileState.screenReaderMode) {
          const message = uiState.showInventory ? 'Closing inventory' : 'Opening inventory';
      ...
      case 'Q':
        ...
        if (mobileState.screenReaderMode) {
          const message = uiState.showQuotes ? 'Closing quotes' : 'Opening saved quotes';
          announceToScreenReader(message);
        }
        break;
      case 'm':
      case 'M':
        ...
        if (mobileState.screenReaderMode) {
          const message = uiState.showWorldMap ? 'Closing world map' : 'Opening world map';
          announceToScreenReader(message);
        }
        break;
      case 'f':
      case 'F':
        ...
        if (mobileState.screenReaderMode) {
          const message = uiState.showFeedback ? 'Closing feedback form' : 'Opening feedback form';
          announceToScreenReader(message);
        }
        break;
      case 'Escape':
      ...
      case 'H':
        ...
        setMobileState(prev => ({ ...prev, highContrastMode: !prev.highContrastMode }));
        if (mobileState.screenReaderMode) {
          const message = mobileState.highContrastMode ? 'High contrast mode disabled' : 'High contrast mode enabled';
          announceToScreenReader(message);
        }
        break;
      case 'r':
      case 'R':
        ...
        setMobileState(prev => ({ ...prev, reducedMotionMode: !prev.reducedMotionMode }));
        if (mobileState.screenReaderMode) {
          const message = mobileState.reducedMotionMode ? 'Reduced motion mode disabled' : 'Reduced motion mode enabled';
          announceToScreenReader(message);
        }
        break;
      case 's':
      case 'S':
        ...
        setMobileState(prev => ({ ...prev, screenReaderMode: !prev.screenReaderMode }));
        const message = mobileState.screenReaderMode ? 'Screen reader mode disabled' : 'Screen reader mode enabled';
        announceToScreenReader(message);
        break;
    }
  ...
  const initSoundManager = useCallback(async () => {
    ...
    try {
      ...
      // Start playing music based on current map
      const currentMapName = currentMap?.name || '';
      if (currentMapName === 'Yosemite') {
        manager.playMusic('yosemite', true, 0.3);
      } else if (currentMapName.includes('Overworld')) {
        manager.playMusic('overworld', true, 0.3);
      } else if (currentMapName.includes('Dungeon')) {
        manager.playMusic('terminal', true, 0.3);
      }
    } catch (error) {
      console.error('Error initializing sound manager:', error);
    }
  }, [soundManager, currentMap]);
  ...
  useEffect(() => {
    ...
    if (characterMovement.movementDirection) {
      ...
      if (characterMovement.movementDirection === 'up' || characterMovement.movementDirection === 'down') {
        updateCharacterState({ verticalDirection: characterMovement.movementDirection });
      } else if (characterMovement.movementDirection === 'left' || characterMovement.movementDirection === 'right') {
        updateCharacterState({ horizontalDirection: characterMovement.movementDirection });
      }
      ...
      const timeout = setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (!characterMovement.diagonalMovement || (characterMovement.diagonalMovement.x === 0 && characterMovement.diagonalMovement.y === 0)) {
            // Only reset directions if we're actually stopping and not continuing to move
            updateCharacterState({ verticalDirection: null, horizontalDirection: null });
          }
        }, 200);
      }, 400);
  ...
  }, [characterPosition, characterMovement, updateCharacterState]);
  ...
  useEffect(() => {
    ...
    const checkBothArtifactSources = () => {
      ...
      const serverArtifact = gameData.artifacts && Array.isArray(gameData.artifacts) ? gameData.artifacts.find(
  ...
  const handleHemingwayComplete = useCallback(() => {
    ...
    // Give rewards, etc.
    updateNotifications({ currentAchievement: 'Completed Hemingway\'s Adventure' });
  ...
  const handleTextAdventureComplete = useCallback(() => {
    setCurrentSpecialWorld(null);
    // Give rewards, etc.
    updateNotifications({ currentAchievement: 'Completed The Writer\'s Journey' });
    updateUIState({ showRewardModal: true });
  }, [setCurrentSpecialWorld, updateNotifications, updateUIState]);
  ...
  const checkForArtifactAchievements = useCallback((updatedInventory) => {
    checkCollectionAchievements(updatedInventory.length);
  }, [checkCollectionAchievements]);

  // Update the handleCreateArtifact function to include achievements
  const handleCreateArtifact = useCallback(async (artifactData) => {
    try {
      ...
      if (character && character.id) {
        const createdCount = gameData.artifacts.filter(a => a.createdBy === character.id).length + 1;
        if (createdCount >= 5) {
  ...
  const handleArtifactClick = useCallback((artifactId) => {
    ...
    const clickedArtifact = gameData.artifacts.find(artifact => artifact.id === artifactId);
    if (!clickedArtifact) return;
    
    // Check if this artifact was already viewed
    const isFirstView = !gameData.viewedArtifacts.includes(artifactId);
    
    // Update viewed artifacts
    if (isFirstView) {
      const updatedViewedArtifacts = [...gameData.viewedArtifacts, artifactId];
  ...
  }, [gameData.artifacts, gameData.viewedArtifacts, updateGameState, updateGameProgress, user, inventory, characterPosition, currentMapIndex, awardXP, checkDiscoveryAchievements, setVisibleArtifact]);

  // Remove an XP notification
  const removeXpNotification = useCallback((id) => {
    updateNotifications({ xpNotifications: prev => prev.filter(notification => notification.id !== id) });
  }, [updateNotifications]);
  
  // Remove an achievement notification
  const removeAchievementNotification = useCallback((id) => {
    updateNotifications({ achievementNotifications: prev => prev.filter(notification => notification.id !== id) });
  }, [updateNotifications]);
  ...
  const handleTileClick = useCallback((x, y, tileType) => {
    ...
    const clickedArtifact = gameData.artifacts.find(artifact => 
      artifact.location && 
  ...
  }, [gameData.artifacts, handleNPCInteraction, handleArtifactClick]);
  ...
  }, [isLoggedIn, character, inventory, gameData.viewedArtifacts, characterPosition, currentMapIndex, updateGameProgress]);
  ...
  const artifactsToShow = useMemo(() => {
    if (!gameData.artifacts || !MAPS || !MAPS[currentMapIndex]) {
      return [];
    }
    
    const currentMapName = MAPS[currentMapIndex].name;
    const artifactMapName = gameData.artifacts.find(artifact => artifact.location)?.location.mapName;
    ...
    <ErrorBoundary>
      ...
      >
        ...
        <div id="game-instructions" className="sr-only">
          Use arrow keys or WASD to move. Press I for inventory, M for map, F for feedback, T to talk to NPCs, 
          H for high contrast mode, R for reduced motion, S for screen reader mode. Press Escape to close menus.
        </div>
        
        {/* Screen reader announcements */}
        <div id="screen-reader-announcements" aria-live="polite" aria-atomic="true" className="sr-only"></div>
        ...
        >
          <div
            ...
            style={{
              transform: mobileState.reducedMotionMode ? `translate(${-viewport.x}px, ${-viewport.y}px)` : `translate(${-viewport.x}px, ${-viewport.y}px)`,
          ...
          >
            ...
            <div className="game-controls">
              <IconButton onClick={() => updateUIState({ showInventory: true })} tooltip="Inventory">
                <i className="fas fa-briefcase"></i>
              </IconButton>
              <IconButton onClick={() => updateUIState({ showWorldMap: true })} tooltip="World Map">
                <i className="fas fa-map"></i>
              </IconButton>
              <IconButton onClick={() => updateUIState({ showQuotes: true })} tooltip="Saved Quotes">
                <i className="fas fa-quote-right"></i>
              </IconButton>
              <IconButton onClick={() => updateUIState({ showWorldGuide: true })} tooltip="World Guide">
                <i className="fas fa-compass"></i>
              </IconButton>
              <IconButton onClick={toggleArtifactsVisibility} tooltip={uiState.showArtifactsOnMap ? "Hide Artifacts" : "Show Artifacts"}>
                <i className={`fas fa-${uiState.showArtifactsOnMap ? 'eye-slash' : 'eye'}`}></i>
              </IconButton>
            </div>
          </div>
        </div>
        ...
        {uiState.showFeedback && (
          <FeedbackForm onClose={() => updateUIState({ showFeedback: false })} />
        )}
        ...
        {uiState.showWinNotification && (
          <div className="win-notification">
            <div className="win-content">
              ...
              <button onClick={() => updateUIState({ showWinNotification: false })}>Continue</button>
            </div>
          </div>
        )}
        ...
        {/* Feedback button */}
        <div className="feedback-button" onClick={() => updateUIState({ showFeedback: true })}>
          <span role="img" aria-label="Feedback">💬</span>
          <span className="feedback-text">Feedback</span>
        </div>

        {/* Map key hint - only shown when there's no other overlay */}
        {!uiState.showInventory && !uiState.showForm && !uiState.showQuotes && !uiState.showWorldMap && !uiState.showWinNotification && !uiState.showRewardModal && !uiState.showLevel4 && !uiState.showFeedback && !uiState.showNPCDialog && (
          <div className="map-key-hint" role="status" aria-live="polite">
            {mobileState.isMobile ? (
              <span>Tap and drag to move | Tap NPCs to talk | Use menu buttons</span>
            ) : (
              <span>Press M to view World Map | Press F for Feedback | Press T to talk to NPCs</span>
            )}
          </div>
        )}
        ...
        {mobileState.showTouchControls && mobileState.touchControlsEnabled && (
          <div className="mobile-controls" role="group" aria-label="Mobile game controls">
            <button
            ...
            <button
              ...
              onClick={() => setMobileState(prev => ({ ...prev, screenReaderMode: !prev.screenReaderMode }))}
              aria-label={mobileState.screenReaderMode ? "Disable screen reader mode" : "Enable screen reader mode"}
            >
              <span role="img" aria-hidden="true">🔊</span>
            </button>
          </div>
