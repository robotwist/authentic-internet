...
import { setMasterVolume, toggleMusic, getMusicConfig } from '../utils/musicMixer';
...
const AudioControls = ({ className = '', setEffectsVolume }) => {
  ...
  const [musicVolume, setMusicVolume] = useState(initialMusicConfig.masterVolume);
  const [sfxVolume, setSfxVolume] = useState(0.5); // Default sound effects volume
  const [musicEnabled, setMusicEnabled] = useState(initialMusicConfig.musicEnabled);
  ...
  return (
    <div className={`audio-controls ${className} ${isOpen ? 'open' : 'closed'}`}>
      <button 
