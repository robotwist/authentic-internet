...
const Dungeon = ({
  ...
  useEffect(() => {
    if (currentRoom.enemies && currentRoom.enemies.length > 0) {
      ...
      if (!defeatedRooms.has(currentRoom.id)) {
        setRoomEnemies(currentRoom.enemies.map((enemy, index) => ({
          ...enemy,
  ...
  // Handle enemy defeat
  const handleEnemyDefeat = useCallback((enemyId, drops) => {
    setRoomEnemies(prev => {
  ...
  const handleItemCollection = useCallback((itemType, itemId) => {
  ...
  const checkDoorInteraction = useCallback((newX, newY) => {
    const tileX = Math.floor(newX / TILE_SIZE);
    const tileY = Math.floor(newY / TILE_SIZE);
    
    // Check if player is at a door
    Object.entries(currentRoom.doors || {}).forEach(([direction, doorData]) => {
      let isDoorTile = false;
      
      // Check door position based on direction
      if (direction === 'north' && tileY === 0 && tileX === Math.floor(currentRoom.width / 2)) {
        isDoorTile = true;
      } else if (direction === 'south' && tileY === currentRoom.height - 1 && tileX === Math.floor(currentRoom.width / 2)) {
        isDoorTile = true;
      } else if (direction === 'east' && tileX === currentRoom.width - 1 && tileY === Math.floor(currentRoom.height / 2)) {
        isDoorTile = true;
      } else if (direction === 'west' && tileX === 0 && tileY === Math.floor(currentRoom.height / 2)) {
        isDoorTile = true;
      }
        ...
        if (doorData.locked) {
          if (doorData.requiresBossKey) {
            ...
            if (playerKeys > 0) {
              ...
              console.log(`🔑 Small key used! Remaining keys: ${playerKeys - 1}`);
              setUnlockedDoors(prev => new Set([...prev, doorKey]));
              transitionToRoom(doorData.leadsTo, direction);
  ...
  const transitionToRoom = useCallback((roomId, entryDirection) => {
  ...
  const renderItems = () => {
    ...
    return currentRoom.items
      .filter(item => {
        ...
        const roomCleared = defeatedRooms.has(currentRoom.id) || !currentRoom.enemies || currentRoom.enemies.length === 0;
