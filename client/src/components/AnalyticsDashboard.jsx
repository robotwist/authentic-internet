...
const AnalyticsDashboard = ({ artifacts, loading = false }) => {
  ...
  const [selectedMetrics, setSelectedMetrics] = useState(['engagement', 'creation', 'discovery']);
  ...
  const analyticsData = useMemo(() => {
    ...
    const overview = {
      ...
      totalCreators: new Set(artifacts.map(a => a.createdBy).filter(Boolean)).size,
      activeCreators: new Set(recentArtifacts.map(a => a.createdBy).filter(Boolean)).size,
      averageRating: artifacts.reduce((sum, a) => sum + (a.rating || 0), 0) / artifacts.length || 0,
      totalViews: artifacts.reduce((sum, a) => sum + (a.viewCount || 0), 0),
      totalReviews: artifacts.reduce((sum, a) => sum + (a.reviews?.length || 0), 0)
    };
  ...
  const calculateTrends = (artifacts, daysBack) => {
    ...
    for (let i = daysBack - 1; i >= 0; i--) {
      ...
      trends.push({
        ...
        creators: new Set(dayArtifacts.map(a => a.createdBy).filter(Boolean)).size,
        averageRating: dayArtifacts.length > 0 
          ? dayArtifacts.reduce((sum, a) => sum + (a.rating || 0), 0) / dayArtifacts.length 
          : 0,
        totalViews: dayArtifacts.reduce((sum, a) => sum + (a.viewCount || 0), 0)
      });
    }
  ...
  const analyzeUserBehavior = (artifacts) => {
    ...
    artifacts.forEach(artifact => {
      if (artifact.type) {
        behavior.preferredTypes[artifact.type] = (behavior.preferredTypes[artifact.type] || 0) + 1;
      }
    });
    ...
    artifacts.forEach(artifact => {
      if (artifact.area) {
        behavior.preferredAreas[artifact.area] = (behavior.preferredAreas[artifact.area] || 0) + 1;
      }
    });
    ...
    artifacts.forEach(artifact => {
      if (artifact.createdBy) {
        behavior.preferredCreators[artifact.createdBy] = (behavior.preferredCreators[artifact.createdBy] || 0) + 1;
      }
    });
          ...
          <Card className="metric-card">
            <CardContent>
              ...
              <Typography variant="body2" className="metric-change positive">
                <TrendingUp /> +{analyticsData.overview.recentArtifacts} this period
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card className="metric-card">
            <CardContent>
              ...
              <Typography variant="body2" className="metric-change positive">
                <TrendingUp /> +{analyticsData.overview.activeCreators - (analyticsData.overview.totalCreators - analyticsData.overview.activeCreators)} vs previous
              </Typography>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card className="metric-card">
            <CardContent>
              ...
              <Typography variant="body2" className="metric-change">
                <AccessTime /> Based on {analyticsData.overview.totalReviews} reviews
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} lg={6}>
          <Card className="chart-card">
            <CardContent>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <RechartsBarChart data={Object.entries(analyticsData.contentPerformance).map(([type, data]) => ({
                  type,
                  ...
                  />
                  <Legend />
                  <Bar dataKey="averageRating" fill="#667eea" name="Avg Rating" />
            ...
            <CardContent>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <RechartsBarChart data={analyticsData.geographicData}>
                  ...
                  <Bar dataKey="averageRating" fill="#f5576c" name="Avg Rating" />
                </RechartsBarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <CardContent>
          ...
          <Grid container spacing={2}>
            {analyticsData.topContent.slice(0, 6).map((artifact, index) => (
              <Grid item xs={12} sm={6} md={4} key={artifact.id || artifact._id}>
