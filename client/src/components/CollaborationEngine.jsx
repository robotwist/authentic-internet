...
const CollaborationEngine = ({ 
  ...
  const [artifactData, setArtifactData] = useState(initialArtifactData || {
    name: '',
    description: '',
    type: 'GAME',
    content: '',
  ...
  useEffect(() => {
    if (autoSaveEnabled && session?.settings?.autoSave) {
      const interval = setInterval(() => {
        if (lastChangeRef.current && Date.now() - lastChangeRef.current > 5000) {
          saveProgress();
        }
  ...
  const handleUserEditing = (data) => {
    setEditingUsers(prev => new Map(prev).set(data.userId, {
  ...
  const publishArtifact = async () => {
    try {
      ...
      const response = await fetch(`/api/collaboration/sessions/${sessionId}/publish`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
