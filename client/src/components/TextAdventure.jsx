...
const TextAdventure = ({ onComplete, onExit, username = "traveler" }) => {
  ...
  const GAME_WORLD = {
    ...
    hallway: {
      description:
        "You enter a long hallway with portraits of famous authors on the walls. You recognize Hemingway, Fitzgerald, and Austen among them. The hallway extends east, and there's a locked door to the west.",
      exits: {
        south: "start",
        east: "library",
        west: "lockedRoom",
    ...
    library: {
      description:
        "You enter a vast library. Shelves stacked with countless books reach toward the high ceiling. A large desk sits in the center, with an open book glowing with strange symbols.",
      exits: {
        west: "hallway",
        north: "studyRoom",
      },
    ...
    studyRoom: {
      description:
        "A cozy study with a fireplace and comfortable armchairs. There's a writing desk with papers scattered across it, and a typewriter that seems to be typing by itself.",
      exits: {
        south: "library",
      },
    ...
    lockedRoom: {
      description:
        "You enter a small study with bookshelves lining the walls. A writing desk faces a window that overlooks an endless sea. This must have been Hemingway's private writing room.",
      exits: {
        east: "hallway",
      },
    ...
    secretPassage: {
      description:
        'A narrow, dimly lit passage leads downward. The walls are inscribed with famous literary quotes. The path spirals down to a heavy oak door with a small plaque that reads "Sanctuary of Truth".',
      exits: {
        up: "hallway",
        door: "sanctuary",
      },
      ...
      lockedExits: {
        door: {
          ...
          message:
            'The door remains closed. Perhaps you should speak the password you learned in the library. (Try: "speak [password]")',
    ...
    sanctuary: {
      description:
        "You enter a circular room with a high domed ceiling painted with constellations. In the center stands a pedestal with a glowing manuscript. Around the room are shadowy figures - the ghosts of great writers past, watching you expectantly.",
      exits: {
        back: "secretPassage",
      },
  ...
  useEffect(() => {
    ...
    const initializeAudio = async () => {
      try {
        ...
        await soundManagerRef.current.loadMusic(
          "goldberg1",
          "/assets/music/text-adventure/goldberg-variations-8bit.mp3",
        );
        await soundManagerRef.current.loadMusic(
          "goldberg2",
          "/assets/music/text-adventure/goldberg-variations-8bit-2.mp3",
        );
      ...
      try {
        ...
        if (gameProgress?.textAdventureProgress) {
          ...
          if (taProgress.inventory && taProgress.inventory.length > 0) {
            ...
            console.log(
              `Loaded ${taProgress.inventory.length} inventory items`,
            );
          }
          ...
          if (
            ...
            console.log(
              `Loaded ${taProgress.completedInteractions.length} completed interactions`,
            );
          }
          ...
          if (
            ...
            console.log(
              `Loaded ${taProgress.knownPasswords.length} pieces of knowledge`,
            );
          }
  ...
  const handleCommand = (e) => {
    ...
    if (command === "help") {
      ...
      addToHistory(
        "system",
        "Tip: If you're stuck, try examining objects in the room or reading items.",
      );
      return;
    }
    ...
    if (currentRoom === "secretPassage") {
      ...
      if (passwordCommands.includes(command)) {
        if (GAME_WORLD.library.playerKnowledge?.includes("password")) {
          addToHistory(
            "system",
            'You speak the words "Iceberg Theory" clearly, and the door responds with a soft glow. It swings open, revealing the sanctuary beyond.',
          );
          ...
          addToHistory(
            "system",
            "You speak the words, but nothing happens. Perhaps you need to learn their significance first.",
          );
          return;
        }
      }
    ...
    if (command.startsWith("use ") && command.includes(" on ")) {
      ...
      if (room.interactions && room.interactions[interactionKey]) {
        ...
        if (interactionData.prerequisite) {
          if (Array.isArray(interactionData.prerequisite)) {
            const allMet = interactionData.prerequisite.every((prereq) => {
              return (
                inventory.includes(prereq) ||
                room.completedInteractions?.includes(prereq)
              );
            });
            ...
            !inventory.includes(interactionData.prerequisite) &&
        ...
        addToHistory(
          "room",
          interactionData.dynamicText
            ? interactionData.dynamicText(inventory)
        ...
        if (interactionData.revealExit) {
          ...
          addToHistory(
            "system",
            `You discovered a new exit: ${interactionData.revealExit}.`,
          );
        }
      ...
      for (const interaction in room.interactions) {
        if (
          ...
          (command.startsWith("examine ") &&
            interaction === `examine ${command.split(" ")[1]}`) ||
          (command.startsWith("look at ") &&
            interaction === `examine ${command.split(" ")[2]}`) ||
          (command.startsWith("take ") &&
            interaction === `take ${command.split(" ")[1]}`) ||
          (command.startsWith("read ") &&
            interaction === `read ${command.split(" ")[1]}`) ||
          (command.startsWith("use ") &&
            interaction === `use ${command.split(" ")[1]}`) ||
          (command.startsWith("talk to ") &&
            interaction === `talk to ${command.slice(8)}`) ||
          (command.startsWith("speak ") &&
            interaction === `speak ${command.split(" ")[1]}`) ||
          (command.startsWith("say ") &&
            interaction === `speak ${command.split(" ")[1]}`) ||
          (command.startsWith("pull ") &&
            interaction === `pull ${command.split(" ")[1]}`) ||
          (command.startsWith("push ") &&
            interaction === `push ${command.split(" ")[1]}`) ||
          (command.startsWith("open ") &&
            interaction === `open ${command.split(" ")[1]}`) ||
          (command.startsWith("write ") &&
            interaction === `write ${command.split(" ")[1]}`)
        ) {
          ...
          if (interactionData.prerequisite) {
            if (Array.isArray(interactionData.prerequisite)) {
              const allMet = interactionData.prerequisite.every((prereq) => {
                return (
                  inventory.includes(prereq) ||
                  GAME_WORLD[currentRoom].completedInteractions?.includes(
                    prereq,
                  )
                );
              });
          ...
          if (interactionData.removeFromRoom) {
            interactionData.removeFromRoom.forEach((item) => {
              GAME_WORLD[currentRoom].items = GAME_WORLD[
                currentRoom
          ...
          if (interactionData.revealExit) {
            ...
            GAME_WORLD[currentRoom].revealedExits.push(
              interactionData.revealExit,
            );
            addToHistory(
              "system",
              `You discovered a new exit: ${interactionData.revealExit}.`,
          ...
          if (interactionData.addKnowledge) {
            ...
            GAME_WORLD[currentRoom].playerKnowledge.push(
              interactionData.addKnowledge,
            );
          }
  ...
  const handleMovement = (direction) => {
    ...
    if (room.lockedExits && room.lockedExits[direction]) {
      ...
      if (lockedExit.key && inventory.includes(lockedExit.key)) {
        addToHistory(
          "system",
          `You use the ${lockedExit.key} to unlock the door.`,
        );
      ...
      if (lockedExit.password && lockedExit.knowledgeRequired) {
        if (
          GAME_WORLD[currentRoom].playerKnowledge?.includes(
            lockedExit.knowledgeRequired,
          )
        ) {
          addToHistory(
            "system",
            `The door responds to the password: "${lockedExit.password}". It swings open.`,
          );
  ...
  const handleEnding = () => {
    ...
    addToHistory(
      "system",
      "As you write your truth on the manuscript, the room begins to fill with a warm, golden light.",
    );
    addToHistory(
      "system",
      "The spirits of the writers nod in approval, and Hemingway's ghost steps forward.",
    );
    addToHistory(
      "system",
      '"You have found the courage to write honestly," he says. "That is the greatest gift a writer can possess."',
    );
    addToHistory(
      "system",
      "The manuscript glows brightly, then transforms into a completed book - your story.",
    );
    addToHistory(
      "system",
      'Hemingway hands you the book. "Take this knowledge back to your world. Write true, write clear, write hard."',
    );
    addToHistory(
      "system",
      "Congratulations! You have completed The Writer's Journey!",
    );
      ...
      {gameComplete && (
        <div className="game-complete-overlay">
          <div className="game-complete-content">
            ...
            <p>
              Your newfound literary wisdom will serve you well in your creative
              endeavors.
            </p>
      ...
      {gameOver && (
        <div className="game-over-overlay">
          <div className="game-over-content">
            <h2>Game Over</h2>
            <p>
              Your inspiration has run dry, and your journey comes to an end.
            </p>
