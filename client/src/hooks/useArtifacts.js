...
import { fetchArtifacts, createArtifact, updateArtifact, deleteArtifact } from '../api/api';
...
export const useArtifacts = () => {
  ...
  const loadArtifacts = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Error loading collected artifacts from localStorage:", err);
      }
    } catch (err) {
  ...
  const collectArtifact = useCallback((artifact) => {
    ...
    setCollectedArtifacts(prev => {
      const updated = [...prev, { ...artifact, collectedAt: new Date().toISOString() }];
  ...
  // Get artifact by ID
  const getArtifactById = useCallback((id) => {
