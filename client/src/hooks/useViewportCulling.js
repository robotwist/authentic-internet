...
export function useViewportCulling(
  ...
  const visibleRange = useMemo(() => {
    ...
    const startX = Math.max(0, Math.floor(-viewport.x / tileSize) - bufferTiles);
    const startY = Math.max(0, Math.floor(-viewport.y / tileSize) - bufferTiles);
    ...
    const endY = Math.min(mapRows, startY + tilesInViewHeight + (bufferTiles * 2));
