...
function gameStateReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.SET_VIEWPORT:
      return { ...state, viewport: resolvePayload(action.payload, state.viewport) };
    ...
    case ACTIONS.UPDATE_UI_STATE:
      return {
        ...
        uiState: {
          ...state.uiState,
          ...resolvePayload(action.payload, state.uiState),
        },
      };
    case ACTIONS.SET_DUNGEON_STATE: {
      const updates = resolvePayload(action.payload, state.dungeonState);
      return {
        ...state,
        dungeonState: { ...state.dungeonState, ...updates },
      };
    }
    case ACTIONS.SET_GAME_DATA: {
      const updates = resolvePayload(action.payload, state.gameData);
      return { ...state, gameData: { ...state.gameData, ...updates } };
    }
    case ACTIONS.SET_PORTAL_STATE: {
      const updates = resolvePayload(action.payload, state.portalState);
      return {
        ...state,
        portalState: { ...state.portalState, ...updates },
      };
    }
    ...
    case ACTIONS.SET_CHARACTER_STATS: {
      const updates = resolvePayload(action.payload, state.characterStats);
      return {
        ...state,
        characterStats: { ...state.characterStats, ...updates },
      };
    }
    ...
    case ACTIONS.SET_MOBILE_STATE: {
      const updates = resolvePayload(action.payload, state.mobileState);
      return {
        ...state,
        mobileState: { ...state.mobileState, ...updates },
      };
    }
    ...
    case ACTIONS.SET_SHOW_GAME_LAUNCHER:
      return {
        ...state,
        showGameLauncher: resolvePayload(action.payload, state.showGameLauncher),
      };
    ...
    case ACTIONS.SET_SHOW_HAMLET_FINALE:
      return {
        ...state,
        showHamletFinale: resolvePayload(action.payload, state.showHamletFinale),
      };
