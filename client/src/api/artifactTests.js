...
export const testArtifactUpdate = async () => {
  ...
  try {
    ...
    if (!isServerAvailable) {
      console.error('❌ Cannot proceed with artifact tests - Server not available');
      return false;
    }
...
const testBasicUpdate = async () => {
  ...
  try {
    ...
    const result = await updateArtifact(TEST_ARTIFACT_ID, TEST_UPDATE_DATA, { testMode: true });
...
const testDirectFetchUpdate = async () => {
  ...
  try {
    ...
    const baseUrl = connectionStatus.activeUrl || 
                   import.meta.env.VITE_API_URL || 
                   'http://localhost:5000';
    
    console.log(`Making direct fetch request to: ${baseUrl}/api/artifacts/${TEST_ARTIFACT_ID}?testMode=true`);
    ...
    const response = await fetch(`${baseUrl}/api/artifacts/${TEST_ARTIFACT_ID}?testMode=true`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({
        ...TEST_UPDATE_DATA,
...
export const testServerConnection = async () => {
  try {
    ...
    const primaryUrl = typeof import.meta !== 'undefined' ? import.meta.env?.VITE_API_URL || 'http://localhost:5000' : 'http://localhost:5000';
    const fallbackUrl = typeof import.meta !== 'undefined' ? import.meta.env?.VITE_API_FALLBACK_URL || 'http://localhost:5001' : 'http://localhost:5001';
    ...
    if (!serverFound) {
      ...
      throw new Error('All connection attempts failed - server may not be running');
    }
