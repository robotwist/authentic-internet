...
export const testArtifactUpdateDirect = async () => {
  ...
  try {
    ...
    const response = await testAxios.put(
      `/api/artifacts/${testArtifactId}?testMode=true`,
      updateData,
    );
...
export const runAuthDiagnostics = async () => {
  ...
  if (sessionToken) {
    console.log(
      "  sessionStorage token details:",
      showTokenDetails(sessionToken),
    );
  }
  ...
  console.log(
    ...
    API.defaults.headers?.common?.["Authorization"] || "Not set",
  );
