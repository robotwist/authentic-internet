...
class GameProgressService {
  ...
  addExperience(amount) {
    if (!amount || amount <= 0) return { experience: this.experience, level: this.level };
  ...
  removeFromInventory(itemId) {
    ...
    const offlineInventory = getGameProgress('offlineInventory', []);
    const updatedOfflineInventory = offlineInventory.filter(item => item.id !== itemId);
  ...
  async completeAchievement(achievementId) {
    ...
    try {
      ...
      const completedAchievements = getGameProgress('completedAchievements', []);
      ...
      // If authenticated, sync with server
      if (getAuthToken()) {
        const response = await API.post(`/api/users/achievements/${achievementId}`);
  ...
  saveQuote(quote) {
    try {
      ...
      if (this.isAuthenticated()) {
        ...
        return API.post(`/api/users/${userId}/quotes`, { quote })
          .then(response => {
    ...
    if (this.isAuthenticated()) {
      ...
      return API.get(`/api/users/${userId}/quotes`)
        .then(response => {
          ...
          const uniqueLocalQuotes = localQuotes.filter(q => !serverQuoteIds.includes(q.id));
  ...
  recordNPCInteraction(npcId) {
    ...
    const npcInteractions = JSON.parse(localStorage.getItem('npcInteractions') || '[]');
    ...
    if (this.isAuthenticated()) {
      ...
      // Use the API object instead of fetch directly
      return API.post(`/api/users/${userId}/npc-interactions`, { npcId })
        .catch(error => {
