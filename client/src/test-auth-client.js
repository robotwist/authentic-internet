...
async function runAuthTests() {
  ...
  try {
    ...
    if (updatedGameState.experience === testGameData.experience) {
      console.log(
        "✅ Experience correctly updated to",
        updatedGameState.experience,
      );
    } else {
      console.log("❌ Experience not updated correctly");
      console.log(
        `Expected: ${testGameData.experience}, Got: ${updatedGameState.experience}`,
      );
    }

    // Check position update
    if (
      updatedGameState.lastPosition.worldId ===
      testGameData.lastPosition.worldId
    ) {
      console.log(
        "✅ World position correctly updated to",
        updatedGameState.lastPosition.worldId,
      );
    } else {
      console.log("❌ World position not updated correctly");
      console.log(
        `Expected: ${testGameData.lastPosition.worldId}, Got: ${updatedGameState.lastPosition.worldId}`,
      );
    }

    // Check quest update
    if (
      updatedGameState.gameProgress.currentQuest ===
      testGameData.gameState.currentQuest
    ) {
      console.log(
        "✅ Current quest correctly updated to",
        updatedGameState.gameProgress.currentQuest,
      );
    } else {
      console.log("❌ Current quest not updated correctly");
      console.log(
        `Expected: ${testGameData.gameState.currentQuest}, Got: ${updatedGameState.gameProgress.currentQuest}`,
      );
    }
