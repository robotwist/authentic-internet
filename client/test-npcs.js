...
async function testGuideNPC() {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "ERROR during Guide interaction:",
...
async function testThaliaTraderNPC() {
  ...
  try {
    const npcsResponse = await api.get("/api/npcs/list");
    const thaliaNPC = npcsResponse.data.find(
      (npc) => npc.name === "Thalia the Trader",
    );
    ...
    try {
      ...
      console.error(
        "ERROR during Thalia interaction:",
        interactionError.message,
      );
      if (interactionError.response) {
        console.error("Response data:", interactionError.response.data);
...
async function runAllTests() {
  ...
  console.log(
    "John Muir NPC Test:",
    johnMuirResult.success ? "✅ PASS" : "❌ FAIL",
  );
  console.log("Guide NPC Test:", guideResult.success ? "✅ PASS" : "❌ FAIL");
  console.log(
    "Thalia the Trader NPC Test:",
    thaliaResult.success ? "✅ PASS" : "❌ FAIL",
  );

  // Overall result
  if (
    npcListResult.success &&
    johnMuirResult.success &&
    guideResult.success &&
    thaliaResult.success
  ) {
